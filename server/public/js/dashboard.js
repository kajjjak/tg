...
function buildStatsMain(pageviews, visits, visitors, newVisitors){
	var plot = $.plot($("#stats-chart2"),
	   [ { data: visits, label: "Customers"},
		 { data: pageviews, label: "Requests"},
		 { data: visitors, label: "Calls" }, 
		 { data: newVisitors, label: "Cancelations"} ], {
		   series: {
			   lines: { show: true,
						lineWidth: 2
					 },
...
$(document).ready(function(){
	...
	if($("#stats-chart2").length) {	
		...
		var url = "/api/client/" + user.company_id + "/report";
		
		$.getJSON(url, function(res){
			var indx, day, c = 0;
			window.total = {"customers": 0, "calls": 0, "request": 0, "canceled": 0};
			for(var d in res["usage"]){
				indx = c;
				pageviews.push([indx, res["usage"][d]["total"]]);
				visits.push([indx, res["usage"][d]["calls"]]);
				visitors.push([indx, res["usage"][d]["request"]]);
				newVisitors.push([indx, res["usage"][d]["canceled"]]);
				total["customers"] = total["customers"] + res["usage"][d]["total"];
				total["calls"] = total["calls"] + res["usage"][d]["calls"];
				total["request"] = total["request"] + res["usage"][d]["request"];
				total["canceled"] = total["canceled"] + res["usage"][d]["canceled"];
				c = c + 1;
				if(c >= 30){ break; }
			}
			for(var i = c; i <= 30; i++){ //fill the rest of missing data with 0
				indx = i;
				pageviews.push([indx, 0]);
				visits.push([indx, 0]);
				visitors.push([indx, 0]);
				newVisitors.push([indx, 0]);
			}
			//render the totals
			for(var i in total){$("#mainstate_total_"+i).html(total[i]);}
			for(var i in total){$("#mainstate_change_"+i).html(total[i]/c);}
			buildStatsMain(pageviews, visits, visitors, newVisitors);
		});
